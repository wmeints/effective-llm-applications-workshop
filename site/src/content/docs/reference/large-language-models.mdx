...
> 📝 **Exercise**: Understanding the LLM training process

<Steps>

1. Learn about the pretraining phase

   - Data is collected from internet and books
   - Words are masked in sentences
   - Model learns to predict missing words
   - This creates base language understanding

2. Understand the fine-tuning phase

   - Model learns specific tasks
   - Examples: writing code, poems, reasoning
   - This creates task-specific capabilities

3. Review the key concepts
   - Pretraining creates base understanding
   - Fine-tuning adds specific abilities
   - The combination enables powerful applications

</Steps>

After this step, the LLM is better at reasoning and completing simple tasks. However,
it does require one more training step to be a reasonable artificial being. 3. The final step in the training process involves aligning the LLM with preferred
...
<Aside>
  **Despite what we told you, there is somewhat of a limit to what you can do
  with tools** You shouldn't provide your agent with 100 tools because it can't
  distinguish between that many tools. Also, don't add too many parameters to
  your tool because that doesn't work either.
</Aside>