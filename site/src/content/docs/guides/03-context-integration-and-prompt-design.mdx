...
<Steps>
1.  Create a new file `apps/chat/InfoSupport.AgentWorkshop.Chat/Tools/QuestionAnsweringTool.cs`. Add the following
    code to it.
...
2.  Create an embedding vector for the question. Modify the `AnswerAsync` method by adding
...
3. Search the vector store
    ...
    relevant chunks. Add the following code to the end of the `AnswerAsync` method.
