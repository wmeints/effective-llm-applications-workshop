...
to obtain the next token is why LLMs are slightly stupid. They can only predict the next
likely token!
</Aside>
...
<Aside>
**Despite what we told you, there is somewhat of a limit to what you can do with tools**  
...
**NOTE** Replace the `<connection string>` placeholder with the connection string you
...
Semantic Kernel is packaged as a Nuget package and can be installed into an ASP.NET Core
...
This installs the core package and two LLM providers: Azure OpenAI, and OpenAI.
You can access other providers by separate installing packages.
...
After installing Semantic Kernel, you need to configure the Kernel object that we discussed in the previous section by
adding the following code to `Program.cs` just below the call `builder.AddServiceDefaults()`.
...
1. First, we configure an Azure OpenAI client and point it to the languagemodel
   connection string.
2. Next, we configure a new kernel object including a chat completion model and
   embedding model.

Note that because we're using Aspire we'll get the connection string for the language
model for free. The `languagemodel` connectionstring is pre-configured in the
application host and automatically injected.

The completion model configured here is used later to generate a response to the user's
prompts. It's important to understand that the chat completion model is the actual LLM
(we choose GPT-4o for the workshop).

The embedding model we've added here we'll need to generate vectors for the search. The
embedding model is not an LLM but rather a model that turns text into a vector
representation. Don't worry if this doesn't make much sense now, we'll explain more
about this later.
...
`ApplicationDbContext` is used to access conversation history information.

Let's take a look at implementing some interaction with our agent. In the `AgentCompletionService` class, we need to define 
an agent that we can talk to. Follow these steps to configure the agent:
...
1. Add the following code at the start of the class:
   ...
   This code defines an agent with a name, instructions, and the kernel instance. The agent
   loads it's instructions from a file called `instructions.txt`.

2. Next, let's look at the `instructions.txt` to understand the agent's instructions. You can
   find the file in the root of the chat project in the `apps/chat/InfoSupport.AgentWorkshop.Chat`
   directory.
...
<Steps>
1. First, we need to retrieve a conversation or create one.
   
   When you're chatting to a digital assistant you typically want to do that in the
   context of a conversation. We're using a `Conversation` class that has a collection
   of messages and some other metadata. You can find the `Conversation` class in
   ...
   **Why are we creating conversations here?**  
   ...
   In the frontend, when we start a new conversation we generate an ID to track client
   state with. We reuse this ID on the server to keep things simple for the client. If
   the user doesn't send any prompt, we'll never create a conversation. No need to track
   empty conversations.
   </Aside>
...
3. Add the following code to the end of the `AgentCompletionService` class to implement
   ...
   <Aside>
   **Now I'm confused, we have threads and chat history?!**  
   Agents come in various shapes. You can have a local agent like the one we're building
   here but you can have an agent that uses the Azure AI Agent Service, finally you can
   also use the OpenAI or Google Agent APIs to build agents. Local agents don't manage
   their own chat history while the cloud services mentioned here manage conversation
   history for you. So if you're using a cloud service to store your agent, you can skip
   the whole conversation management stuff.
   </Aside>
...
4. After creating the thread for the agent, we can record the submitted prompt with the
   ...
   The response stream here is an `IAsyncEnumerable`. An enumerable stream of pieces of
   content generated by the LLM. If you don't like streaming you can use `InvokeAsync`
   instead. For this workshop we use streaming as it provides a nicer user experience.

5. Add the following code to the `GenerateResponseAsync` method to record the assistant
   response.
   
   Our agent doesn't really track chat history so we need to do
   that ourselves. Add the following code to record the response and forward the chunks to
   the caller:
...
6. Add the following code to the `GenerateResponseAsync` method to save the conversation
   with the new user prompt and recorded assistant response.
...
Building LLM-based applications and especially RAG applications involve a lot of moving
parts. First and foremost you have to deal with a language model hosted in the cloud.
Next, you need to record history and store documents in a vector database. Finally, you
need to combine frontend and APIs into a working application.
...
1. **The application host**, which serves as an orchestrator putting all the moving
   parts together so you can run the infrastructure, API, and frontend by starting an
   application host project.
 
2. **The Aspire dashboard**, which exposes metrics, logging, and tracing in a single
   website for all the components in the solution. It makes it a lot easier to debug
...
3. **The service defaults**, this component is a class library containing the necessary
   settings to collect the diagnostics for the dashboard and other general-purpose code
   for the distributed application.

While the main focus of Aspire is on orchestrating distributed applications, you can use
the application model you create in the application host to generate deployment scripts.
For example, you can use the [Azure Developer CLI][AZD] to deploy Aspire-based
application to Azure quite easily.

For the purpose of this workshop, we made sure that the application host and service
defaults are preconfigured for you. We highly recommend copying the code from the
starter folder in each lab to make sure you have the right configuration.
...
In this module we covered the basic concepts of LLMs. We then discussed what Semantic Kernel is
and how the various concepts in LLMs are made easier with Semantic Kernel. Finally, we covered how
to apply the concepts offered by Semantic Kernel to build a basic agent.

In the next module we'll extend the basic agent with the capacity to index documents that the agent
can later search through to give answers to questions that users may have about Semantic Kernel.
